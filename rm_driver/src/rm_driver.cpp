...
void RmArm::Arm_Movej_CANFD_Callback(rm_ros_interfaces::msg::Jointpos::SharedPtr msg)
{
    ...
    RCLCPP_WARN(this->get_logger(), "Movej CANFD, follow: %d, expand: %f, trajectory_mode: %d, radio: %d", follow, expand, trajectory_mode, radio);
...
UdpPublisherNode::UdpPublisherNode():
    rclcpp::Node("udp_publish_node"){
        ...
        /********************************************************************UDP传输数据**********************************************************/
        Joint_Position_Result = this->create_publisher<sensor_msgs::msg::JointState>("arm_joint_states", 10);                                //发布当前的关节角度(相对话题,按命名空间区分)
...
RmArm::RmArm():
    ...
    this->declare_parameter("hand_joint_names", hand_joint_names_);
    this->get_parameter("hand_joint_names", hand_joint_names_);
    ...
    hand_joint_names_g = hand_joint_names_;
